...
1. Запустить `git status` — посмотреть, что изменилось.
2. Запустить `git diff` — понять суть изменений.
3. Определить правильный `тип` и `scope` по таблицам выше.
4. Добавить нужные файлы в индекс (предпочитать конкретные имена файлов вместо `git add -A`).
